...
public class KursServlet extends HttpServlet {
    ...
    private void processReqeust(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (requestURI.endsWith("/login.html")) {
            if ("Register".equals(request.getParameter("button"))) {
                ...
                if (login.isEmpty() || (user = registerBean.register(login, password)).getId() == 0) {
                    request.setAttribute("message", "Registration faild. Login " + user.getLogin() + " is invalid");
        ...
        }else if (requestURI.endsWith("/singnout.html")){
